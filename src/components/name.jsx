...
function Name() {
    ...
    const fetchmeals = async () => {
        ...
        const res = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${searchitem}`);
  ...
  return (
    <div className=' flex flex-col justify-center items-center lg:px-32 px-16' >
       ...
       <div className='w-full relative ml-[50px] mr-[50px] '>
        <input type="search" value={searchitem} onChange={userinput}  placeholder='Search meal by name' onKeyPress={Key} className=' bg-gray-300  pl-10 pr-4 py-2 w-full   border-none rounded-lg focus:outline-none  ' />
       ...
       <div className= 'flex flex-wrap gap-8 justify-center'>
        {meals.map((meal, index) => (
            <div key={index} className='w-full lg:w-1/4 p-5  shadow-[0_3px_10px_rgba(0,0,0,0.2)] rounded-lg'>
                ...
                <p className='font-semibold  mt-[10px] text-[13px]'>{meal.strInstructions.substring(0, 200)}.....</p>
